...
while True:
    ...
    if(dastoor=="PWD"):
        ...
        jomleh=("HELP :  list of known commands \n LIST : list if directory or file in server with size \n PWD :   Return the pathname of the current directory on the server \n"
                "(dirname) MKDIR :   Create a new directory on the server \n (dirname) RMDIR :   Remove the directory named dirname on the server \n"
                " (dirname) CD :   Set the current directory on the server \n (filename) DOWNLOAD :   download file from server \n")
        connectionsocket.send(jomleh.encode())
    elif (dastoor == "LIST"):
        listt = os.listdir()
        listtt=""
